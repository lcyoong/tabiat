<template>
          ...
          <div class="hidden w-full lg:grid lg:grid-cols-7 lg:grid-rows-6z lg:gap-px">
            <div v-for="day in days" :key="day.date" :class="[day.isCurrentMonth ? 'bg-white' : 'bg-gray-50 text-gray-500', 'relative py-2 px-3 h-24']">
              ...
              <div class="flex gap-0.5">
                <div v-for="event in day.events" class="flex h-1.5 w-1.5 items-center justify-center rounded-full bg-indigo-600 font-semibold text-white"></div>
              </div>
  ...
  //   return prop.goal.habits.flatMap(habit => habit.tracks.map(track => track))
  // })
  ...
  //   { date: '2022-01-01', isfCurrentMonth: true, events: [] },
